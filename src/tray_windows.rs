...
pub fn run_tray() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let menu = Menu::new();
    let autostart_item =
        CheckMenuItem::with_id("autostart", "Start at login", true, autostart_on, None);
    ...
    let _tray = TrayIconBuilder::new()
        .with_tooltip(APP_SLUG)
        .with_icon(icon)
        .with_menu(Box::new(menu))
        .build()?;

    run_message_loop(&autostart_item, exit_item.id(), &exe);
    ...
    unsafe {
        ...
        while GetMessageW(&mut msg, std::ptr::null_mut(), 0, 0) > 0 {
            ...
            while let Ok(event) = menu_events.try_recv() {
                if event.id == *autostart.id() {
                    handle_autostart_toggle(autostart, exe);
